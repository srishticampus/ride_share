...
const RiderForgotPass = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await service.FindUserPh(phoneNumber);
            console.log(response);
            
            if (response.status === 'success') {
                // toast.success("Phone number verified! Redirecting...");
                alert("Phone number verified! Redirecting...")
                console.log('Rider found:', response.data);
                setTimeout(() => {
                    navigate(`/User-Reset-Pass/${response.data.phoneNumber}`)
                }, 2000);
    
            } else {
                // toast.error(response.message || 'No rider found with this phone number');
                alert(response.message || 'No rider found with this phone number')
            }
